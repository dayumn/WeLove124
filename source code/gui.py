...
def setup_editor_shortcuts(text_input, parent_window, tab_widget, font_family):
    ...
    # Zoom shortcuts - using standard sequences
    ...
    # Additional explicit zoom shortcuts
    if IS_MACOS:
        # Cmd+= for zoom in (= is on same key as + without shift)
        zoom_in_equal = QShortcut(QKeySequence("Cmd+="), text_input)
        zoom_in_equal.activated.connect(text_input.zoomIn)
        
        # Cmd+Shift+= for zoom in (this is Cmd++)
        zoom_in_shift = QShortcut(QKeySequence("Cmd+Shift+="), text_input)
        zoom_in_shift.activated.connect(text_input.zoomIn)
        
        # Cmd+- for zoom out
        zoom_out_minus = QShortcut(QKeySequence("Cmd+-"), text_input)
        zoom_out_minus.activated.connect(text_input.zoomOut)
    else:
        # Ctrl+= for zoom in
        zoom_in_equal = QShortcut(QKeySequence("Ctrl+="), text_input)
        zoom_in_equal.activated.connect(text_input.zoomIn)
        
        # Ctrl+Shift+= for zoom in
        zoom_in_shift = QShortcut(QKeySequence("Ctrl+Shift+="), text_input)
        zoom_in_shift.activated.connect(text_input.zoomIn)
        
        # Ctrl+- for zoom out
        zoom_out_minus = QShortcut(QKeySequence("Ctrl+-"), text_input)
        zoom_out_minus.activated.connect(text_input.zoomOut)
...
def create_new_tab(tab_widget, parent_window, font_family, file_name=None, content=None):
    ...
    # create custom close button on the right
...
def execute_code(tab_widget, lexeme_manager, token_table, 
    ...
    # Create worker thread with filename
    filename = file_manager.file_name if file_manager.file_name else None
    worker = InterpreterWorker(content_manager.saved_content, console_widget, filename=filename)
